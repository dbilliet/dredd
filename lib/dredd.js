...
var CONNECTION_ERRORS, Dredd, Runner, applyConfiguration, async, blueprintAstToRuntime, blueprintUtils, configureReporters, fs, glob, handleRuntimeProblems, logger, options, protagonist, removeDuplicates, request, url,
  ...
  Dredd.prototype.run = function(callback) {
    ...
    parseBlueprints = function(cb) {
      return async.each(Object.keys(config.data), function(file, parseCallback) {
        return protagonist.parse(config.data[file]['raw'], function(protagonistError, result) {
          if (protagonistError) {
